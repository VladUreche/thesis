...
\topic{The most complicated and time-consuming part of our work involved rewriting the miniboxing runtime support to match the performance of specialized code.} The next subsections present the HotSpot Java Virtual Machine execution (\S\ref{subsec-mb-jvm}), the main benchmark we used for testing (\S\ref{subsec-mb-bench}) and two implementations for the runtime support: type byte switching (\S\ref{subsec-mb-switching}) and object-oriented dispatching (\S\ref{subsec-mb-dispatching}).
